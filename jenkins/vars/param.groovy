...
def getAgentsTableScriptHTML(Agents, ELIGIBLE_AGENTS){ 
    def html_node_options   = {default_label-> """<select name="value" >""" + ([default_label]+ ( (ELIGIBLE_AGENTS.collect { it.trim() }).remove(default_label))).collect{ item -> return "<option value=$item>$item "}.join(' ') + "</select>"}
